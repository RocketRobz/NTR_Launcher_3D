...
ROMFS		:=	romfs
APP_AUTHOR	:=	Robz8
APP_DESCRIPTION :=  DS(i) boot screen in 3D
ICON		:=	app/icon.png
...
# More external tools
#---------------------------------------------------------------------------------
CITRA 	?= D:\emulators\citra\citra.exe $(TARGET).3dsx
...
CFLAGS	:=	-g -Wall -Wstrict-aliasing -O2 -DVERSION_MAJOR=$(VERSION_MAJOR) -DVERSION_MINOR=$(VERSION_MINOR) -DVERSION_MICRO=$(VERSION_MICRO) -mword-relocations \
		-ffunction-sections \
		$(ARCH)
...
clean:
	@echo clean ...
	@rm -fr $(BUILD) $(TARGET).3dsx $(TARGET).cia $(TARGET).elf
...
cia: $(OUTPUT).elf $(OUTPUT).cia
3dsx: $(OUTPUT).elf $(OUTPUT).3dsx
...
$(OUTPUT).cia	:	$(OUTPUT).elf  $(OUTPUT).smdh
	# $(BANNERTOOL) makebanner -i "../app/banner.png" -a "../app/BannerAudio.bcwav" -o "../app/banner.bin"
